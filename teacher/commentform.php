 ...
 echo' <div class="col-sm-12 formBg">
    ...
    Comment
     ...
     echo"<div class='col-sm-10 col-sm-offset-1'>";
        ...
        switch ($_GET['s']) {
          case '0':
          echo "<p class='error'> failed to insert comment.please try again or contact admin.</p>";
            break;

             case '1':
          echo "<p class='success'>  comment Input Successfull .</p>";
            break;

            case '2':
          echo "<p class='error'>  you did not insert or update any comment .</p>";
            break;
        ...
        echo" <a href='comment.php' class='btn btn-success'>Go Back To Add Attendance </a>
        </div>";
         }
...
<form class='form-group' method='post' action='insert_comment.php'>";
foreach ($student_info as $key => $value) {

$s++;

$r= select_comment($value['student_id'],$value['student_no'],$_SESSION['class_id'],$_SESSION['subclass'],$value['reg_id'],$_SESSION['term'],$_SESSION['session']);
$i++;
$no =count($r);
if(count($r) == 0){
//e#
