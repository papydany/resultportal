...
function  footer(){
  ...
  <p style="margin:auto;">Copyright &copy;&nbsp;&nbsp;<?php  echo date("Y")."&nbsp;&nbsp;"; ?>All Right Reserved</p>
...
<div class="navbar navbar-default navbar-fixed-top " role="navigation">
        <div class="navbar-header">
          ...
          <div class="navbar-collapse  collapse  col-sm-offset-1 " id="ccc" >
            ...
            <li><img src="images/padge.png" alt="school logo"  class="img-responsive"/></li>
...
?>
  <div class="row topbanner">
    <div class="col-sm-10 col-sm-offset-1">
        ...
        <div class="col-sm-9 toptext col-sm-offset-3">
                <span> TDDC SCHOOLS & COLLEGE RESULT PORTAL</span>
            </div>
        </div>
  </div>
  <?php
...
if($term =='First Term' || $term =='Second Term'){
...
foreach ($a1 as $k1 => $v1) {
  ...
  $reg = get_reg_id($v1,$sc,$c,$term,$s);
$rs = select_result_display($v1,$c,$sc,$reg['reg_id'],$sb_id,$term,$s);
if($reg){
$a = select_sub_total($v1,$c,$sc,$sb_id,$term,$s); // total marks function for first term
$af = $v1;
}
...
while ($r1 =mysqli_fetch_assoc($result1)) {
  $reg = get_reg_id($r1['student_id'],$sc,$c,$term,$s);
  if($reg){
  $a1[] =$r1['student_id'];
  }
}
...
function select_schedule($c){
  $conn =db();
  $query ="Select * from fess_schedules where id ='$c'";
  $result =mysqli_query($conn,$query) or die(mysqli_error($conn));
  $r =mysqli_fetch_assoc($result);
  return $r;
}
